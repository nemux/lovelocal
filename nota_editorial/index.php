...
if(!empty($p_not)):
	...
	endwhile;
	$ruta="http://lovelocal.mx/nota_editorial/?not=".$id_nota."&nota=".$nota;
	$ruta_imagen="";
	$ancho_imagen="";
	$alto_imagen="";
...
$c_per=buscar("per.id_perfil
  ...
  $ruta_perfil="http://lovelocal.mx/perfil/?per=".$id_perfil."&perfil=".$local;
  $ruta_imagen="";
  if(!empty($imagen_perfil) and file_exists("../administrador/perfiles/images/".$imagen_articulo)):
    //$ruta_imagen=urlencode("http://lovelocal.mx/administrador/perfiles/images/".$imagen_perfil); 
    $ruta_imagen="http://lovelocal.mx/administrador/perfiles/images/".$imagen_perfil;  
  endif;
//
  $direccion="";
$direccion_mostrar="";
...
<head> 
<title><? if(!empty($nota)): echo $nota." - ";endif;?>LoveLocal</title> 
<!-- Favicon -->
...
//if(!empty($ruta_imagen)):
	?>
   <meta property="og:image" content="http://lovelocal.mx/administrador/notas_editoriales/images/<?php echo $imagen_nota; ?>" />
   <?
                          ...
                          <?
								  if(!empty($imagen_nota) and file_exists("../administrador/notas_editoriales/images/".$imagen_nota)):
									  ...
									  <div class="entry-thumbnail divider_40">
										<img class="" src="http://lovelocal.mx/administrador/notas_editoriales/images/<?=$imagen_nota?>" alt="<?=$nota?>" rel="image_src">
										</div>
										<?
									endif;
                  ?>
...
<div class='shareaholic-canvas' data-app='share_buttons' data-app-id='25891808'></div>
                  ...
                  <!--Perfil-->
                  <a class="lnkperfil" href="../perfil/?per=<?=$id_perfil?>&perfil=<?=$local?>"  title="<?=$local?>" >
                  <?php
                  if(!empty($imagen_perfil) and file_exists("../administrador/perfiles/images/".$imagen_perfil)):
                    ?>
                                       
                                           <img src="http://lovelocal.mx/administrador/perfiles/images/<?=$imagen_perfil?>" class="attachment-shop_single wp-post-image" alt="<?=$local?>" title="<?=$local?>" rel="image_src">
                  ...
                  <div class="detailperfil">  
                    ...
                    <p><?php echo $local; ?></p>
                    <p class="subtitle_resumen">
                    <strong>Direccion: </strong><?=substr($direccion_mostrar,2)?><br />
                    ...
                    <img src="http://lovelocal.mx/images/linedetail.jpg"/> 
                  </div>
                  <div class="lnkperfil">IR AL PERFIL</div>
                  </a>
                  <!--Promociones-->
      <?php
                if ($id_perfil_nota > 0)
            ...
            ,est.estado
            ,p.codigo_postal,
            pro.id_perfil,pro.id_promocion,pro.promocion,pro.imagen,pro.descripcion_corta","promociones_perfiles pro, perfiles p, estados est","WHERE pro.id_perfil = '".$id_perfil_nota."' AND  pro.estatus='Activo'  AND pro.id_perfil=p.id_perfil AND p.id_estado = est.id_estado ORDER BY pro.id_promocion DESC limit 3",false);
                }
                else
                { 
                //Buscar promociones
            ...
            ,est.estado
            ,p.codigo_postal,
            pro.id_perfil,pro.id_promocion,pro.promocion,pro.imagen,pro.descripcion_corta","promociones_perfiles pro, perfiles p, estados est","WHERE pro.estatus='Activo'  AND pro.id_perfil=p.id_perfil AND p.id_estado = est.id_estado ORDER BY RAND() limit 3",false);
                }
                              if($c_pro->num_rows>0):
                                 ?>
