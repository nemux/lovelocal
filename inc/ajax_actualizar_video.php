...
if(!empty($se_id_perfil) and !empty($p_id_video) and empty($errors)):
	...
	$c_art=buscar("titulo,video","videos_perfiles","WHERE id_video='".$p_id_video."' AND id_perfil='".$se_id_perfil."'");
	while($f_art=$c_art->fetch_assoc()):
		$titulo=cadena($f_art["titulo"]);
		$video=cadena($f_art["video"]);
	endwhile;
	?>
	<style>
   ...
   <form id="form_videos" action="../inc/ajax_videos_actualizar.php" method="post" enctype="multipart/form-data">
   ...
   <div class="row">
	...
	<div class="row">
		...
		<div class="col-sm-10 col-md-10 col-lg-10">
			<textarea name="video" style="width:100%; height:100px;" required><?=$video?></textarea> *<br> Ej. https://www.youtube.com/watch?v=lmpJNAOIAV4
		</div>
	</div>                           
	<div class="row">
		<div class="col-sm-2 col-md-2 col-lg-2">
      	<input type="hidden" name="id_video" value="<?=$p_id_video?>">
			<input type="hidden" name="action_videos" value="send">
		</div>
		<div class="col-sm-10 col-md-10 col-lg-10">
			<button type="submit" id="form_videos_submit" name="form_videos_submit" class="theme_button">Actualizar video</button>
		</div>
	</div>
	<div class="row">
      <div class="col-sm-2 col-md-2 col-lg-2">
