	...
	if($p_action=="send"):
		...
		if($p_numero>0):
			for($con=1;$con<=$p_numero;$con++):
				$hospital='hospital'.$con;
				$imagen='imagen'.$con;
				//$imagen_original='imagen_original'.$con;
				$orden='orden'.$con;
				$estatus='estatus'.$con;
				
				$p_hospital=cadenabd($_POST[$hospital]);
				...
				if(!empty($p_hospital) and
				!empty($p_estatus)
					):
					//Buscar el ultimo id de la imagen
					$c_gr=buscar("id_hospital","hospitales","order by id_hospital desc limit 1");
					while($f_gr=mysql_fetch_assoc($c_gr)):
						$id_hospital=$f_gr["id_hospital"];
					...
					if(!empty($_FILES["imagen".$con]["name"])):
						...
						if($_FILES['imagen'.$con]['error']==0):
							...
							elseif(!in_array(pathinfo($_FILES["imagen".$con]["name"], PATHINFO_EXTENSION), $valid_formats)):
								...
								$nombre_archivo="img_".$id_hospital."_".$count."_".$fechahora.$extension;
					...
					if(!empty($varname2)):
						...
						$p_imagen_original="io".$id_hospital.$extension2;
					...
					*/
					...
					//Registrar imagenes en hospital
					$i_c=insertar("hospital
										,imagen
										,descripcion_corta
										,orden
										,estatus"
										...
										,'".$p_estatus."'");
					if(!$i_c):
						$arreglo_respuestas[]='<div class="error">No fue posible registrar el hospital '.$con.'</div>';
					else:
						$arreglo_respuestas[]='<div class="exito">Hospital '.$con.' registrado exitosamente.</div>';
					endif;					
				else:
...
<div id="contenedor">
    ...
    <div id="separador">&nbsp;</div>
      ...
      <fieldset>
      <legend>Nuevo Hospital</legend>
                      <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                        <tr>
                        	<td></td>
                           <td>
                              <input id="agregar_hospital" name="agregar_hospital" type="button" class="link_5" value="Agregar hospital" style="clear:both;" />
                           </td>
                        </tr>
                        <tr>
                        	<td colspan="2">
                              <div id="div_hospitales">
                              	<fieldset>
                                 <legend>Hospital 1</legend>
                                 <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                                    <tr>
                                       <td>Hospital</td>
                                       <td><input type="text" name="hospital1" style="width:300px;" maxlength="255" required>*</td>
                                    </tr>
                                    <tr>
                                       <td>Imagen</td>
	...
	<script>
		jQuery(document).ready(function(){
			jQuery("#agregar_hospital").click(function(event){
				var numero=(new Number(jQuery("#numero").val())*1)+1;
				if(numero>0 && (jQuery("#tbl_hospital"+numero).length)==0){
					url ="../agregar_hospital.php";
					var posting = jQuery.post(url,{numero:numero});
					posting.done(function(data){
						jQuery("#div_hospitales").append(data);
