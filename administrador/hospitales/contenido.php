...
if(!empty($p_nombre)):
	...
	if(!empty($p_nombre)):
		$condicion.=" AND (ga.hospital LIKE '%".$p_nombre."%') ";
	endif;
...
if($RegistrosAMostrar=="todos"):
	$orden=" ORDER BY ga.orden,ga.hospital";
else:
	$orden=" ORDER BY ga.orden,ga.hospital LIMIT $RegistrosAEmpezar, $RegistrosAMostrar";
endif;

$condicion2=$condicion.$orden;

$campos="ga.id_hospital,ga.hospital,ga.imagen,ga.descripcion_corta,ga.orden,ga.estatus";
$tablas="hospitales ga ";
...
<table width="100%">
   ...
   <?
	...
	while($f_id=mysql_fetch_assoc($c_id)):
		$contador++;
		$id_hospital=$f_id["id_hospital"];
		$hospital=cadena($f_id["hospital"]);
		...
		?>
      <tr class="txt_center">
      	<td style="text-align:center"><input type="checkbox" name="arreglo_registros[]" value="<?=$id_hospital?>"/></td>
         <td><?=$hospital?></td>
         <td>
				<?
