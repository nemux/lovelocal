		...
		for($i=0;$i<count($p_arreglo_registros);$i++):
			...
			*/
			...
			if(!empty($p_hospital) and
				...
				$c_se=buscar("imagen","hospitales","where id_hospital=".$registro);
				...
				*/
				...
				$actualizar="hospital='".$p_hospital."'";
				...
				*/
				$actualizar.=",descripcion_corta='".$p_descripcion_corta."'
								,orden=".$p_orden."
								,estatus='".$p_estatus."'";
				$c_id=actualizar("".$actualizar."","hospitales","where id_hospital=".$registro."");
				
				if(!$c_id):
					$arreglo_respuestas[]='<div class="error">No fue posible actualizar este hospital '.$registro.'</div>';
				else:
					$arreglo_respuestas[]='<div class="exito">Hospital '.$registro.' actualizado exitosamente.';
				endif;				
			else:
    ...
    <div id="separador">&nbsp;</div>
      ...
      if(count($arreglo_respuestas)>0):
		...
		if(count($p_arreglo_registros)>0):
			for($i=0;$i<count($p_arreglo_registros);$i++):
				$registro=$p_arreglo_registros[$i];
				$c_id=buscar("hospital
									,imagen
									,descripcion_corta
									,orden
									,estatus"
								,"hospitales"
								,"where id_hospital=".$registro."");
									
				while($f_c=mysql_fetch_assoc($c_id)):
					$hospital=cadena($f_c['hospital']);
					$imagen=$f_c['imagen'];
				...
				?>
      					<fieldset>
                     <legend>Actualizar Hospital</legend>
                      <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                        ...
                        <tr>
                           <td width="20%">Hospital</td>
                           <td width="80%"><input type="text" name="hospital<?=$registro?>" style="width:300px;" maxlength="255" value="<?=$hospital?>"></td>
                        </tr>
                        <?
