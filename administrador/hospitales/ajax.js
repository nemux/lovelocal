...
function cargar_grupos(id_fotografo,id_hospital){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_hospital=new Number(id_hospital)*1;	

	var contenedor;	
	if(id_hospital>0){
		contenedor=document.getElementById('td_grupos'+id_hospital);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_hospital="+id_hospital+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
