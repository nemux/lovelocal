...
if(!empty($p_nombre)):
	...
	if(!empty($p_nombre)):
		$condicion.=" AND (cat.categoria LIKE '%".$p_nombre."%') ";
	endif;
...
if($RegistrosAMostrar=="todos"):
	$orden=" ORDER BY cat.orden,cat.categoria";
else:
	$orden=" ORDER BY cat.orden,cat.categoria LIMIT $RegistrosAEmpezar, $RegistrosAMostrar";
endif;

$condicion2=$condicion.$orden;

$campos="cat.id_categoria,cat.categoria,cat.imagen,cat.descripcion_corta,cat.orden,cat.estatus";
$tablas="categorias cat ";
...
<table width="100%">
   ...
   <?
	...
	while($f_id=$c_id->fetch_assoc()):
		$contador++;
		$id_categoria=$f_id["id_categoria"];
		$categoria=cadena($f_id["categoria"]);
		...
		?>
      <tr class="txt_center">
      	<td style="text-align:center"><input type="checkbox" name="arreglo_registros[]" value="<?=$id_categoria?>"/></td>
         <td><?=$categoria?></td>
         <td>
				<?
