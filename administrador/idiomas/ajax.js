...
function cargar_grupos(id_fotografo,id_idioma){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_idioma=new Number(id_idioma)*1;	

	var contenedor;	
	if(id_idioma>0){
		contenedor=document.getElementById('td_grupos'+id_idioma);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_idioma="+id_idioma+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
