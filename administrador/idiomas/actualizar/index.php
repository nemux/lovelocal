		...
		for($i=0;$i<count($p_arreglo_registros);$i++):
			...
			*/
			...
			if(!empty($p_idioma) and
				...
				$c_se=buscar("imagen","idiomas","where id_idioma=".$registro);
				...
				*/
				$actualizar.=",descripcion_corta='".$p_descripcion_corta."'
								,orden=".$p_orden."
								,estatus='".$p_estatus."'";
				$c_id=actualizar("".$actualizar."","idiomas","where id_idioma=".$registro."");
				
				if(!$c_id):
					$arreglo_respuestas[]='<div class="error">No fue posible actualizar este idioma '.$registro.'</div>';
				else:
					$arreglo_respuestas[]='<div class="exito">Idioma '.$registro.' actualizado exitosamente.</div>';
				endif;				
			else:
		...
		if(count($p_arreglo_registros)>0):
			for($i=0;$i<count($p_arreglo_registros);$i++):
				...
				$c_id=buscar("idioma
									...
									,estatus"
								,"idiomas"
								,"where id_idioma=".$registro."");
									
				while($f_c=mysql_fetch_assoc($c_id)):
					$idioma=cadena($f_c['idioma']);
					$imagen=$f_c['imagen'];
				...
				?>
      					<fieldset>
                     <legend>Actualizar Idioma</legend>
                      <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                        ...
                        <tr>
                           ...
                           <td width="80%"><input type="text" name="idioma<?=$registro?>" style="width:300px;" maxlength="255" value="<?=$idioma?>"></td>
                        </tr>
                        <?
