	...
	if($p_action=="send"):
		...
		if($p_numero>0):
			for($con=1;$con<=$p_numero;$con++):
				...
				$p_idioma=cadenabd($_POST[$idioma]);
				...
				if(!empty($p_idioma) and
					!empty($p_estatus)
					):
					//Buscar el ultimo id de la imagen
					$c_gr=buscar("id_idioma","idiomas","order by id_idioma desc limit 1");
					while($f_gr=mysql_fetch_assoc($c_gr)):
						$id_idioma=$f_gr["id_idioma"];
					...
					if(!empty($_FILES["imagen".$con]["name"])):
						...
						if($_FILES['imagen'.$con]['error']==0):
							...
							elseif(!in_array(pathinfo($_FILES["imagen".$con]["name"], PATHINFO_EXTENSION), $valid_formats)):
								...
								$nombre_archivo="img_".$id_idioma."_".$count."_".$fechahora.$extension;
					...
					if(!empty($varname2)):
						...
						$p_imagen_original="io".$id_idioma.$extension2;
					...
					*/
					...
					$i_c=insertar("idioma
										...
										,'".$p_estatus."'");
					if(!$i_c):
						$arreglo_respuestas[]='<div class="error">No fue posible registrar el idioma '.$con.'</div>';
					else:
						$arreglo_respuestas[]='<div class="exito">Idioma '.$con.' registrado exitosamente.</div>';
					endif;					
				else:
...
<div id="contenedor">
      ...
      <legend>Nuevo Idioma</legend>
                      <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                        <tr>
                        	<td></td>
                           <td>
                              <input id="agregar_idioma" name="agregar_idioma" type="button" class="link_5" value="Agregar idioma" style="clear:both;" />
                           </td>
                        </tr>
                        <tr>
                        	<td colspan="2">
                              <div id="div_idiomas">
                              	<fieldset>
                                 <legend>Idioma 1</legend>
                                 <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                                    <tr>
                                       <td>Idioma</td>
                                       <td><input type="text" name="idioma1" style="width:300px;" maxlength="255" required>*</td>
                                    </tr>
                                    <tr>
                                       <td>Imagen</td>
	...
	<script>
		jQuery(document).ready(function(){
			jQuery("#agregar_idioma").click(function(event){
				var numero=(new Number(jQuery("#numero").val())*1)+1;
				if(numero>0 && (jQuery("#tbl_idioma"+numero).length)==0){
					url ="../agregar_idioma.php";
					var posting = jQuery.post(url,{numero:numero});
					posting.done(function(data){
						jQuery("#div_idiomas").append(data);
