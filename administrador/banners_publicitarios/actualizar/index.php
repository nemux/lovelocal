			...
			for($i=0;$i<count($p_arreglo_registros);$i++):
				...
				*/
				...
				if(!empty($p_banner) and
					...
					$c_se=buscar("imagen","banners_publicitarios","where id_banner=".$registro);
					...
					*/
					$actualizar.=",enlace='".$p_enlace."'
									,orden=".$p_orden."
									,estatus='".$p_estatus."'";
					$c_id=actualizar("".$actualizar."","banners_publicitarios","where id_banner=".$registro."");
					
					if(!$c_id):
						$arreglo_respuestas[]='<div class="error">No fue posible actualizar este banner '.$registro.'</div>';
					else:
						$arreglo_respuestas[]='<div class="exito">Banner publicitario '.$registro.' actualizada exitosamente</div>';
					endif;				
				else:
			...
			if(count($p_arreglo_registros)>0):
				for($i=0;$i<count($p_arreglo_registros);$i++):
					...
					$c_id=buscar("banner
									...
									,"where id_banner=".$registro."");
										
					while($f_c=$c_id->fetch_assoc()):
						$banner=cadena($f_c['banner']);
						$imagen=$f_c['imagen'];
					...
					?>
								<fieldset>
								<legend>Actualizar Banner publicitario</legend>
								 <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
									...
									<tr>
										<td width="20%">Banner publicitario</td>
										<td width="80%"><input type="text" name="banner<?=$registro?>" style="width:300px;" maxlength="255" value="<?=$banner?>"></td>
									</tr>
									<?
									if(!empty($imagen) and file_exists("../images/".$imagen)):
										...
										<tr>
										  ...
										  <td class="text_gral"><input type="file" name="imagen<?=$registro?>"/>
												<br />
												<span>Le sugerimos una imagen con 1752 x 250 pixeles y formato JPG</span>
												</td>	   
										</tr>
										<?
									else:
									...
									<tr>
										<td>Enlace</td>
										<td>
											<textarea name="enlace<?=$registro?>" id="enlace<?=$registro?>" style="width:600px;height:100px;"><?=$enlace?></textarea>
										</td>
									</tr>                        
									<tr>
										<td>Orden</td>
