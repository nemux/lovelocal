...
function buscar_banners(id_division,id_idioma,id_seccion,identificador){
	var aleatorio= generar_aleatorio();
	...
	var contenedor2;
	var respuesta,secciones,banners;
	if(identificador>0){
		contenedor2=document.getElementById('td_banners'+identificador);
	}
	else{
		contenedor2=document.getElementById('td_banners');
	}
	ajax=objetoAjax();
	ajax.open("GET", "../buscar_banners.php?id_division="+id_division+"&id_idioma="+id_idioma+"&id_seccion="+id_seccion+"&identificador="+identificador+"&aleatorio="+aleatorio);
	contenedor2.innerHTML='<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			respuesta=ajax.responseText.split("|");
			banners=respuesta[0];
			contenedor2.innerHTML=banners;
		}
	}
	ajax.send(null)
}
...
function cargar_grupos(id_fotografo,id_banner){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_banner=new Number(id_banner)*1;	

	var contenedor;	
	if(id_banner>0){
		contenedor=document.getElementById('td_grupos'+id_banner);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_banner="+id_banner+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
