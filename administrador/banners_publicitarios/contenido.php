...
if(!empty($p_nombre)):
	...
	if(!empty($p_nombre)):
		$condicion.=" AND (ban.banner LIKE '%".$p_nombre."%') ";
	endif;
...
if($RegistrosAMostrar=="todos"):
	$orden=" ORDER BY ban.orden,ban.banner";
else:
	$orden=" ORDER BY ban.orden,ban.banner LIMIT $RegistrosAEmpezar, $RegistrosAMostrar";
endif;

$condicion2=$condicion.$orden;

$campos="ban.id_banner,ban.banner,ban.imagen,ban.enlace,ban.orden,ban.estatus";
$tablas="banners_publicitarios ban ";
...
<table width="100%">
   ...
   <?
	...
	while($f_id=$c_id->fetch_assoc()):
		$contador++;
		$id_banner=$f_id["id_banner"];
		$banner=cadena($f_id["banner"]);
		...
		?>
      <tr class="txt_center">
      	<td style="text-align:center"><input type="checkbox" name="arreglo_registros[]" value="<?=$id_banner?>"/></td>
         <td><?=$banner?></td>
         <td>
				<?
