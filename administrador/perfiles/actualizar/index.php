...
if($p_action=="send"):
	...
	if(count($p_arreglo_registros)>0):
		...
		for($i=0;$i<count($p_arreglo_registros);$i++):
			...
			$p_servicios=$_POST["servicios".$registro];
			if(!empty($p_perfil)and
				...
				if($total>0):
					...
					if(!$c_id):
						...
						$c_cat=eliminar("productos_perfiles","WHERE id_perfil='".$registro."' AND tipo='Servicio'");
						if(count($p_servicios)):
							foreach($p_servicios as $servicio):
								//Registrarlo
								$c_serv=insertar("tipo, producto ,id_perfil","productos_perfiles","'Servicio','".cadenabd($servicio)."','".$registro."'");
							endforeach;
						endif;
						//CATEGORIAS
						//Borrar si es que existe alguno
      ...
      <?
		if(count($p_arreglo_registros)>0):
			for($i=0;$i<count($p_arreglo_registros);$i++):
				...
				endwhile;
				//Buscar servicios
				$c_serv=buscar("producto","productos_perfiles","WHERE id_perfil='".$registro."'");
				$arreglo_servicios=array();
				while($f_serv=$c_serv->fetch_assoc()):
					$arreglo_servicios[]=$f_serv["producto"];
				endwhile;
				?>
            ...
            ?>
               ...
               <table border="0" cellpadding="3" cellspacing="1" align="left" style="width:100%;">
                  <tr>
                     <td colspan="2">
                     <input type="checkbox" name="servicios<?=$registro?>[]" value="Accesible para personas con discapacidad" <? if(in_array('Accesible para personas con discapacidad',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Accesible para personas con discapacidad
                        	</div>
							<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Wifi" <? if(in_array('Wifi',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Wifi
                        	</div>
         					<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Conexión eléctrica (para cargar aparatos eléctricos)" <? if(in_array('Conexión eléctrica (para cargar aparatos eléctricos)',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Enchufes
                        	</div> 
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Estacionamiento" <? if(in_array('Estacionamiento',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Estacionamiento
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Valet Parking" <? if(in_array('Valet Parking',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Valet Parking
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Servicio a Domicilio" <? if(in_array('Servicio a Domicilio',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Servicio a Domicilio
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Pet Friendly" <? if(in_array('Pet Friendly',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Pet Friendly
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Sección de Fumar" <? if(in_array('Sección de Fumar',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Sección de Fumar
                        	</div>
         					<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Vinos y Cervezas" <? if(in_array('Vinos y Cervezas',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Vinos y Cervezas
                        	</div>
         					<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Bar" <? if(in_array('Bar',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Bar
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Sommelier" <? if(in_array('Sommelier',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Sommelier
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Tarjeta de Débito y Crédito" <? if(in_array('Tarjeta de Débito y Crédito',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Tarjeta de Débito y Crédito
                        	</div>
         					<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Música" <? if(in_array('Música',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Música
                        	</div>
         					<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Televisión" <? if(in_array('Televisión',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Televisión
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Entretenimiento en vivo" <? if(in_array('Entretenimiento en vivo',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Entretenimiento en vivo
                        	</div>
         					<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Terraza o jardín" <? if(in_array('Terraza o jardín',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Terraza o jardín
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Banquete" <? if(in_array('Banquete',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Banquete
                        	</div>
                        	<div style="display:inline-block; margin-right:15px; margin-bottom:7px;">
                        	<input type="checkbox" name="servicios<?=$registro?>[]" value="Bebidas y café" <? if(in_array('Bebidas y café',$arreglo_servicios)): ?> checked="checked" <? endif;?>> Bebidas y café
                        	</div>
