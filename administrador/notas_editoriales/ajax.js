...
function buscar_notas(id_division,id_idioma,id_seccion,identificador){
	var aleatorio= generar_aleatorio();
	...
	var contenedor2;
	var respuesta,secciones,notas;
	if(identificador>0){
		contenedor2=document.getElementById('td_notas'+identificador);
	}
	else{
		contenedor2=document.getElementById('td_notas');
	}
	ajax=objetoAjax();
	ajax.open("GET", "../buscar_notas.php?id_division="+id_division+"&id_idioma="+id_idioma+"&id_seccion="+id_seccion+"&identificador="+identificador+"&aleatorio="+aleatorio);
	contenedor2.innerHTML='<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			respuesta=ajax.responseText.split("|");
			notas=respuesta[0];
			contenedor2.innerHTML=notas;
		}
	}
	ajax.send(null)
}
...
function cargar_grupos(id_fotografo,id_nota){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_nota=new Number(id_nota)*1;	

	var contenedor;	
	if(id_nota>0){
		contenedor=document.getElementById('td_grupos'+id_nota);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_nota="+id_nota+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
