			...
			for($i=0;$i<count($p_arreglo_registros);$i++):
				...
				*/
				...
				if(!empty($p_categoria) and
					...
					$c_se=buscar("imagen","categorias","where id_categoria=".$registro);
					...
					*/
					...
					$actualizar="categoria='".$p_categoria."'";
					...
					*/
					$actualizar.=",descripcion_corta='".$p_descripcion_corta."'
									,orden=".$p_orden."
									,estatus='".$p_estatus."'";
					$c_id=actualizar("".$actualizar."","categorias","where id_categoria=".$registro."");
					
					if(!$c_id):
						$arreglo_respuestas[]='<div class="error">No fue posible actualizar esta categoria '.$registro.'</div>';
					else:
						$arreglo_respuestas[]='<div class="exito">Categoría '.$registro.' actualizada exitosamente</div>';
					endif;				
				else:
	...
	<div id="contenedor">
		...
		<div id="contenido">
			...
			if(count($p_arreglo_registros)>0):
				for($i=0;$i<count($p_arreglo_registros);$i++):
					$registro=$p_arreglo_registros[$i];
					$c_id=buscar("categoria
										,imagen
										,descripcion_corta
										,orden
										,estatus"
									,"categorias"
									,"where id_categoria=".$registro."");
										
					while($f_c=$c_id->fetch_assoc()):
						$categoria=cadena($f_c['categoria']);
						$imagen=$f_c['imagen'];
								...
								<legend>Actualizar Categoría</legend>
								 <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
									...
									<tr>
										...
										<td width="80%"><input type="text" name="categoria<?=$registro?>" style="width:300px;" maxlength="255" value="<?=$categoria?>"></td>
									</tr>
									<?
