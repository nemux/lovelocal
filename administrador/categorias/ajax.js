...
function cargar_grupos(id_fotografo,id_categoria){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_categoria=new Number(id_categoria)*1;	

	var contenedor;	
	if(id_categoria>0){
		contenedor=document.getElementById('td_grupos'+id_categoria);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_categoria="+id_categoria+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
