...
function Pagina(nropagina,cuantos,p_nombre){
 ...
 divContenido.innerHTML= '<img src="http://www.ziikfor.com/administrador/images/gifs/loading.gif">';
...
function buscar_secciones(id_division,id_idioma,identificador){
	...
	contenedor.innerHTML='<img src="http://www.ziikfor.com/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
...
function buscar_promociones(id_division,id_idioma,id_seccion,identificador){
	var aleatorio= generar_aleatorio();
	...
	var contenedor2;
	var respuesta,secciones,promociones;
	if(identificador>0){
		contenedor2=document.getElementById('td_promociones'+identificador);
	}
	else{
		contenedor2=document.getElementById('td_promociones');
	}
	ajax=objetoAjax();
	ajax.open("GET", "../buscar_promociones.php?id_division="+id_division+"&id_idioma="+id_idioma+"&id_seccion="+id_seccion+"&identificador="+identificador+"&aleatorio="+aleatorio);
	contenedor2.innerHTML='<img src="http://www.ziikfor.com/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			respuesta=ajax.responseText.split("|");
			promociones=respuesta[0];
			contenedor2.innerHTML=promociones;
		}
	}
	ajax.send(null)
}
...
function cargar_grupos(id_fotografo,id_promocion){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_promocion=new Number(id_promocion)*1;	

	var contenedor;	
	if(id_promocion>0){
		contenedor=document.getElementById('td_grupos'+id_promocion);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_promocion="+id_promocion+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.ziikfor.com/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
