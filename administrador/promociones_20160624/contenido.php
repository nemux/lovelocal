...
if(!empty($p_nombre)):
	...
	if(!empty($p_nombre)):
		$condicion.=" AND (pro.promocion LIKE '%".$p_nombre."%') ";
	endif;
...
if($RegistrosAMostrar=="todos"):
	$orden=" ORDER BY pro.orden,pro.promocion";
else:
	$orden=" ORDER BY pro.orden,pro.promocion LIMIT $RegistrosAEmpezar, $RegistrosAMostrar";
endif;

$condicion2=$condicion.$orden;

$campos="pro.id_promocion,pro.promocion,pro.imagen,pro.descripcion_corta,pro.orden,pro.estatus";
$tablas="promociones pro ";
...
<table width="100%">
   ...
   <?
	...
	while($f_id=$c_id->fetch_assoc()):
		$contador++;
		$id_promocion=$f_id["id_promocion"];
		$promocion=cadena($f_id["promocion"]);
		...
		?>
      <tr class="txt_center">
      	<td style="text-align:center"><input type="checkbox" name="arreglo_registros[]" value="<?=$id_promocion?>"/></td>
         <td><?=$promocion?></td>
         <td>
				<?
