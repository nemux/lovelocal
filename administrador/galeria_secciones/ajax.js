...
function buscar_titulos(id_division,id_idioma,id_seccion,identificador){
	var aleatorio= generar_aleatorio();
	...
	var contenedor2;
	var respuesta,secciones,titulos;
	if(identificador>0){
		contenedor2=document.getElementById('td_titulos'+identificador);
	}
	else{
		contenedor2=document.getElementById('td_titulos');
	}
	ajax=objetoAjax();
	ajax.open("GET", "../buscar_titulos.php?id_division="+id_division+"&id_idioma="+id_idioma+"&id_seccion="+id_seccion+"&identificador="+identificador+"&aleatorio="+aleatorio);
	contenedor2.innerHTML='<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			respuesta=ajax.responseText.split("|");
			titulos=respuesta[0];
			contenedor2.innerHTML=titulos;
		}
	}
	ajax.send(null)
}
...
function cargar_grupos(id_fotografo,id_foto){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_foto=new Number(id_foto)*1;	

	var contenedor;	
	if(id_foto>0){
		contenedor=document.getElementById('td_grupos'+id_foto);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_foto="+id_foto+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
