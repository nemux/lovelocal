...
if(!empty($p_nombre)):
	...
	if(!empty($p_nombre)):
		$condicion.=" AND (ga.padecimiento LIKE '%".$p_nombre."%') ";
	endif;
...
if($RegistrosAMostrar=="todos"):
	$orden=" ORDER BY ga.orden,ga.padecimiento";
else:
	$orden=" ORDER BY ga.orden,ga.padecimiento LIMIT $RegistrosAEmpezar, $RegistrosAMostrar";
endif;

$condicion2=$condicion.$orden;

$campos="ga.id_padecimiento,ga.padecimiento,ga.imagen,ga.descripcion_corta,ga.orden,ga.estatus";
$tablas="padecimientos ga ";
...
<table width="100%">
   ...
   <?
	...
	while($f_id=mysql_fetch_assoc($c_id)):
		$contador++;
		$id_padecimiento=$f_id["id_padecimiento"];
		$padecimiento=cadena($f_id["padecimiento"]);
		...
		?>
      <tr class="txt_center">
      	<td style="text-align:center"><input type="checkbox" name="arreglo_registros[]" value="<?=$id_padecimiento?>"/></td>
         <td><?=$padecimiento?></td>
         <td>
				<?
