...
function cargar_grupos(id_fotografo,id_padecimiento){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_padecimiento=new Number(id_padecimiento)*1;	

	var contenedor;	
	if(id_padecimiento>0){
		contenedor=document.getElementById('td_grupos'+id_padecimiento);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_padecimiento="+id_padecimiento+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
