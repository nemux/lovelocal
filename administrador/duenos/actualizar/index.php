	...
	if(count($p_arreglo_registros)>0):
		...
		for($i=0;$i<count($p_arreglo_registros);$i++):
			...
			if(!empty($p_perfil)and
				...
				$r_c=buscar("COUNT(*) AS total","duenos","WHERE usuario='".$p_usuario."' 
							AND 	password='".$p_password."'
							and	id_dueno<>".$registro."");
				while($f_c=$r_c->fetch_assoc()):
					$total=$f_c['total'];
				endwhile;
				if($total>0):
					$arreglo_respuestas[]='<div class="error">Ya existe un dueño con este usuario '.cadena($p_usuario).'</div>';
				else:
					$c_se=buscar("imagen","duenos","where id_dueno=".$registro);
					...
					$actualizar.=",calle='".$p_calle."'
									...
									,estatus='".$p_estatus."'
									";
					$c_id=actualizar("".$actualizar."","duenos","where id_dueno=".$registro);
					if(!$c_id):
						$arreglo_respuestas[]='<div class="error">No fue posible actualizar este dueño '.cadena($p_usuario).'</div>';
					else:
						$arreglo_respuestas[]='<div class="exito">Dueño '.cadena($p_usuario).' actualizado exitosamente.</div>';
					endif;
				endif;
...
<title>DUEÑOS DE LOCALES O SERVICIOS - ACTUALIZAR</title>
 <!-- Favicon -->
