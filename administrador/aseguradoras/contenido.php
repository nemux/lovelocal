...
if(!empty($p_nombre)):
	...
	if(!empty($p_nombre)):
		$condicion.=" AND (ga.aseguradora LIKE '%".$p_nombre."%') ";
	endif;
...
if($RegistrosAMostrar=="todos"):
	$orden=" ORDER BY ga.orden,ga.aseguradora";
else:
	$orden=" ORDER BY ga.orden,ga.aseguradora LIMIT $RegistrosAEmpezar, $RegistrosAMostrar";
endif;

$condicion2=$condicion.$orden;

$campos="ga.id_aseguradora,ga.aseguradora,ga.imagen,ga.descripcion_corta,ga.orden,ga.estatus";
$tablas="aseguradoras ga ";
...
<table width="100%">
   ...
   <?
	...
	while($f_id=mysql_fetch_assoc($c_id)):
		$contador++;
		$id_aseguradora=$f_id["id_aseguradora"];
		$aseguradora=cadena($f_id["aseguradora"]);
		...
		?>
      <tr class="txt_center">
      	<td style="text-align:center"><input type="checkbox" name="arreglo_registros[]" value="<?=$id_aseguradora?>"/></td>
         <td><?=$aseguradora?></td>
         <td>
				<?
