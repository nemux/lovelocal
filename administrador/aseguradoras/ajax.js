...
function cargar_grupos(id_fotografo,id_aseguradora){
	var aleatorio= generar_aleatorio();
	var id_fotografo=new Number(id_fotografo)*1;
	var id_aseguradora=new Number(id_aseguradora)*1;	

	var contenedor;	
	if(id_aseguradora>0){
		contenedor=document.getElementById('td_grupos'+id_aseguradora);
	}
	else{
	...
	ajax=objetoAjax();
	ajax.open("GET", "../cargar_grupos.php?id_fotografo="+id_fotografo+"&id_aseguradora="+id_aseguradora+"&aleatorio="+aleatorio);
	contenedor.innerHTML= '<img src="http://www.lovelocal.mx/administrador/images/gifs/loading.gif">';
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4){
