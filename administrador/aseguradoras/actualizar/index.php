...
if($p_action=="send"):
	...
	if(count($p_arreglo_registros)>0):
		...
		for($i=0;$i<count($p_arreglo_registros);$i++):
			...
			$aseguradora="aseguradora".$registro;
			...
			*/
			...
			if(!empty($p_aseguradora) and
				...
				$c_se=buscar("imagen","aseguradoras","where id_aseguradora=".$registro);
				...
				*/
				...
				$actualizar="aseguradora='".$p_aseguradora."'";
				...
				*/
				$actualizar.=",descripcion_corta='".$p_descripcion_corta."'
								,orden=".$p_orden."
								,estatus='".$p_estatus."'";
				$c_id=actualizar("".$actualizar."","aseguradoras","where id_aseguradora=".$registro."");
				
				if(!$c_id):
					$arreglo_respuestas[]='<div class="error">No fue posible actualizar esta aseguradora '.$registro.'.</div>';
				else:
					$arreglo_respuestas[count($arreglo_respuestas)]='<div class="exito">Aseguradora '.$registro.' actualizada exitosamente.</div>';
				endif;				
			else:
    ...
    <div id="separador">&nbsp;</div>
      ...
      <?
		...
		if(count($p_arreglo_registros)>0):
			for($i=0;$i<count($p_arreglo_registros);$i++):
				$registro=$p_arreglo_registros[$i];
				$c_id=buscar("aseguradora
									,imagen
									,descripcion_corta
									,orden
									,estatus"
								,"aseguradoras"
								,"where id_aseguradora=".$registro."");
									
				while($f_c=mysql_fetch_assoc($c_id)):
					$aseguradora=cadena($f_c['aseguradora']);
					$imagen=$f_c['imagen'];
				...
				?>
      					<fieldset>
                     <legend>Actualizar Aseguradora</legend>
                      <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                        ...
                        <tr>
                           ...
                           <td width="80%"><input type="text" name="aseguradora<?=$registro?>" style="width:300px;" maxlength="255" value="<?=$aseguradora?>"></td>
                        </tr>
                        <?
