	...
	if($p_action=="send"):
		...
		if($p_numero>0):
			for($con=1;$con<=$p_numero;$con++):
				$aseguradora='aseguradora'.$con;
				$imagen='imagen'.$con;
				//$imagen_original='imagen_original'.$con;
				$orden='orden'.$con;
				$estatus='estatus'.$con;
				
				$p_aseguradora=cadenabd($_POST[$aseguradora]);
				...
				if(!empty($p_aseguradora) and
					!empty($p_estatus)
					):
					//Buscar el ultimo id de la imagen
					$c_gr=buscar("id_aseguradora","aseguradoras","order by id_aseguradora desc limit 1");
					while($f_gr=mysql_fetch_assoc($c_gr)):
						$id_aseguradora=$f_gr["id_aseguradora"];
						$id_aseguradora=$id_aseguradora+1;
					endwhile;
					if(empty($id_aseguradora)):
						$id_aseguradora=1;
					endif;
					...
					if(!empty($_FILES["imagen".$con]["name"])):
						...
						if($_FILES['imagen'.$con]['error']==0):
							...
							elseif(!in_array(pathinfo($_FILES["imagen".$con]["name"], PATHINFO_EXTENSION), $valid_formats)):
								...
								$nombre_archivo="img_".$id_aseguradora."_".$count."_".$fechahora.$extension;
					...
					if(!empty($varname2)):
						...
						$p_imagen_original="io".$id_aseguradora.$extension2;
					...
					*/
					...
					//Registrar imagenes en aseguradora
					$i_c=insertar("aseguradora
										,imagen
										,descripcion_corta
										,orden
										,estatus"
										...
										,'".$p_estatus."'");
					if(!$i_c):
						$arreglo_respuestas[]='<div class="error">No fue posible registrar la aseguradora '.$con.'</div>';
					else:
						$arreglo_respuestas[]='<div class="exito">Aseguradora '.$con.' registrada exitosamente.</div>';
					endif;						
				else:
...
<div id="contenedor">
    ...
    <div id="separador">&nbsp;</div>
      ...
      <fieldset>
      <legend>Nueva Aseguradora</legend>
                      <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                        <tr>
                        	<td></td>
                           <td>
                              <input id="agregar_aseguradora" name="agregar_aseguradora" type="button" class="link_5" value="Agregar aseguradora" style="clear:both;" />
                           </td>
                        </tr>
                        <tr>
                        	<td colspan="2">
                              <div id="div_aseguradoras">
                              	<fieldset>
                                 <legend>Aseguradora 1</legend>
                                 <table border="0" cellpadding="3" cellspacing="1" width="80%" align="left" class="text_gral">
                                    <tr>
                                       <td>Aseguradora</td>
                                       <td><input type="text" name="aseguradora1" style="width:300px;" maxlength="255" required>*</td>
                                    </tr>
                                    <tr>
                                       <td>Imagen</td>
	...
	<script>
		jQuery(document).ready(function(){
			jQuery("#agregar_aseguradora").click(function(event){
				var numero=(new Number(jQuery("#numero").val())*1)+1;
				if(numero>0 && (jQuery("#tbl_aseguradora"+numero).length)==0){
					url ="../agregar_aseguradora.php";
					var posting = jQuery.post(url,{numero:numero});
					posting.done(function(data){
						jQuery("#div_aseguradoras").append(data);
